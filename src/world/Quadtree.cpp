...
Quadtree::Quadtree(const sf::Vector2f& position, const sf::Vector2f& dimensions, const bool hasChildren, const unsigned char level) :
	...
	m_level(level)
{
	m_polygon.addPoint((sf::Vector2f)position);
	m_polygon.addPoint(sf::Vector2f(position.x + dimensions.x, position.y));
	m_polygon.addPoint(sf::Vector2f(position.x + dimensions.x, position.y + dimensions.y));
	m_polygon.addPoint(sf::Vector2f(position.x, position.y + dimensions.y));
}
...
void Quadtree::insert(std::shared_ptr<math::Polygon> polygon, unsigned char index)
{
	...
	{
		for (int i = 0; i < 4; ++i)
			if (math::polygonIntersectsPolygon(m_children[i]->m_polygon, *polygon))
...
void Quadtree::split()
{
	...
	for (int i = 0; i < 4; ++i)
	{
		for (auto it = m_polygons.begin(); it != m_polygons.end(); ++it)
			if (math::polygonIntersectsPolygon(m_children[i]->m_polygon, *it->second))
...
std::vector<math::Polygon> Quadtree::getPolygons(const math::Polygon& polygon, std::vector<unsigned char>& indices)
{
	...
	if (m_children[0])
	{
		for (int i = 0; i < 4; ++i)
		{
			auto _polygons = m_children[i]->getPolygons(polygon, indices);
			polygons.insert(polygons.end(), _polygons.begin(), _polygons.end());
		}
	}
	else if (math::polygonIntersectsPolygon(polygon, m_polygon))
	{
		for (auto it = m_polygons.begin(); it != m_polygons.end(); ++it)
		{
